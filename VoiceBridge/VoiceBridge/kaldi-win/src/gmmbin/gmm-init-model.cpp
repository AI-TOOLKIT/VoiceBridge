...
namespace kaldi {
		...
		for (size_t i = 0; i < split_stats.size(); i++) {
			if (split_stats[i].empty()) {
				...
				for (int32 idx = 0; idx < bad_phones.size(); idx++)
					ss << bad_phones[idx] << ' ';
				KALDI_WARN << "Tree has pdf-id " << i << " with no stats; corresponding phone list: " << ss.str();
				bNoStats = true; //@+zso
				/*
	...
	int InitAmGmmFromOld(const BuildTreeStatsType &stats,
		...
		if (static_cast<int32>(split_stats.size()) != to_pdf_map.MaxResult() + 1) {
			KALDI_ASSERT(static_cast<int32>(split_stats.size()) < to_pdf_map.MaxResult() + 1);
...
/*
	GmmInitModel
	NOTE: returns 2 in case there are warnings about no stats. 0 if all OK.
*/
int GmmInitModel(int argc, char *argv[], fs::ofstream & file_log) {
	...
	try {
		...
		if (file_log)
			file_log << "Number of separate statistics is " << stats.size() << "\n";
		else KALDI_LOG << "Number of separate statistics is " << stats.size();
		...
		if (file_log)
			file_log << "Wrote model." << "\n";
		else KALDI_LOG << "Wrote model.";

		DeleteBuildTreeStats(&stats);

		//@+zso
		if (bNoStats) return 2;
		else return 0;
	}
