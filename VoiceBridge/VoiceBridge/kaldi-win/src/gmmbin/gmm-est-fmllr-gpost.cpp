...
/*
	GmmEstFmllrGpost : Estimate global fMLLR transforms, either per utterance or for the supplied set of speakers (spk2utt option).
*/
	...
	try {
		...
		if (spk2utt_rspecifier != "") {  // per-speaker adaptation
			...
			for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
				...
				if(file_log)
					file_log << "For speaker " << spk << ", auxf-impr from fMLLR is "
			...
			for (; !feature_reader.Done(); feature_reader.Next()) {
				...
				if (file_log)
					file_log << "For utterancer " << utt << ", auxf-impr from fMLLR is "
		...
		if (file_log) {
			file_log << "Done " << num_done << " files, " << num_no_gpost
				<< " with no gposts, " << num_other_error << " with other errors." << "\n";
			file_log << "Overall fMLLR auxf impr per frame is "
