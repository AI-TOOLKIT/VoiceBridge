...
/*
	GmmEstFmllr : Estimate global fMLLR transforms, either per utterance or for the supplied set of speakers (spk2utt option).
*/
	...
	try {
		...
		if (spk2utt_rspecifier != "") {  // per-speaker adaptation
			...
			for (; !feature_reader.Done(); feature_reader.Next()) {
				...
				if(file_log)
					file_log << "For utterance " << utt << ", auxf-impr from fMLLR is "
		...
		if (file_log) {
			file_log << "Done " << num_done << " files, " << num_no_post
