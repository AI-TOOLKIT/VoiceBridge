...
int ConvertAli(int argc, char *argv[], fs::ofstream & file_log) {
	...
	try {
		...
		if(file_log)
			file_log << "Succeeded converting alignments for " << num_success << " files, failed for " << num_fail << "\n";
		else 
			KALDI_LOG << "Succeeded converting alignments for " << num_success << " files, failed for " << num_fail;
