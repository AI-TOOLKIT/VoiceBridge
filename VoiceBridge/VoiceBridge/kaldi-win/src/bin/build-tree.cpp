	...
	try {
		...
		{ // This block is to warn about low counts.
			...
			for (size_t i = 0; i < split_stats.size(); i++)
				if (SumNormalizer(split_stats[i]) < 100.0)
					KALDI_VLOG(1) << "For pdf-id " << i << ", low count " << SumNormalizer(split_stats[i]);
		}
		...
		{  // This block is just doing some checks.
			...
			if (!unseen_phones.empty()) {
				...
				KALDI_WARN << "In certain kinds of systems e.g. where phone encodes position and stress information having no stats is normal.";
			}
		}

		if (file_log)
			file_log << "Wrote tree" << "\n";
		else KALDI_LOG << "Wrote tree";
