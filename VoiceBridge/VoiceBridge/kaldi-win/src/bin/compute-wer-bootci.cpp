...
/*
	...
	Compute a bootstrapping of WER to extract the 95\% confidence interval. Take a reference and a transcription file, 
	in integer or text format, and outputs overall WER statistics to standard output along with its confidence interval 
	using the bootstrap methos of Bisani and Ney. If a second transcription file corresponding to the same reference is
	provided, a bootstrap comparison of the two transcription is performed to estimate the probability of improvement.
*/
int ComputeWerBootci(int argc, char *argv[], fs::ofstream & file_log) {
  ...
  try {
    const char *usage =
      ...
      "\n"
	  "Usage: compute-wer-bootci [options] <ref-rspecifier> <hyp-rspecifier> <wspecifier> [<hyp2-rspecifier>]\n" //@+zso
    ...
    if (po.NumArgs() < 2 || po.NumArgs() > 4) { //@+zso
		//po.PrintUsage();
		//exit(1);
    ...
    std::string hyp2_rspecifier = (po.NumArgs() == 4?po.GetArg(4):""); //@+zso
	...
	*/
	//@+zso NOTE: ± does not print well on std::cout !
	KALDI_LOG << "Set1: WER " << std::fixed << std::setprecision(2) << 100 * mean_wer << "%" << " +- " << 100 * interval << "%";

	if (!hyp2_rspecifier.empty()) {
		KALDI_LOG << "Set2: WER " << std::fixed << std::setprecision(2) << 100 * mean_wer2 << "%" << " +- " << 100 * interval2 << "%";
		KALDI_LOG << "Probability of Set2 improving Set1: " << std::fixed << std::setprecision(2) << 100 * p_improv;
	}
	...
	if (wspecifier != "") {
		...
		file_ws << "Set1: WER " << std::fixed << std::setprecision(2) << 100 * mean_wer << "%" << " ± " << 100 * interval << "%" << '\n';

		if (!hyp2_rspecifier.empty()) {
			file_ws << "Set2: WER " << std::fixed << std::setprecision(2) << 100 * mean_wer2 << "%" << " ± " << 100 * interval2 << "%" << '\n';

			file_ws << "Probability of Set2 improving Set1: " << std::fixed << std::setprecision(2) << 100 * p_improv << '\n';
		}
