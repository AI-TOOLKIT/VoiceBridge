...
/*
	LatticeToPost : Do forward-backward and collect posteriors over lattices.
*/
int LatticeToPost(int argc, char *argv[], fs::ofstream & file_log) {
	try {
		...
		for (; !lattice_reader.Done(); lattice_reader.Next()) {
			...
			if (file_log)
				file_log << "Processed lattice for utterance: " << key << "; found "
		...
		if (file_log) {
			file_log << "Overall average log-like/frame is "
