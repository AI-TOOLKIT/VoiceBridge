...
/*
	NbestToProns : Reads lattices which must be linear (single path), and must be in CompactLattice form 
	where the transition-ids on the arcs have been aligned with the word boundaries (see lattice-align-words*)
	and outputs a vaguely ctm-like format.
*/
int NbestToProns(int argc, char *argv[], fs::ofstream & file_log) {
	try {
		...
		if(file_log)
			file_log << "Printed prons for " << n_done << " linear lattices; " << n_err << " had errors." << "\n";
		else KALDI_LOG << "Printed prons for " << n_done << " linear lattices; " << n_err << " had errors.";
		return (n_done != 0 ? 0 : 1);
	}
