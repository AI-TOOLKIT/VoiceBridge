...
/*
	LinearToNbest : This does the opposite of nbest-to-linear.  It takes 4 archives, containing alignments, 
	word-sequences, and acoustic and LM costs, and turns it into a single archive containing FSTs with a linear
	structure.
*/
int LinearToNbest(int argc, char *argv[], fs::ofstream & file_log) {
	try {
		...
		if(file_log)
			file_log << "Done " << n_done << " n-best entries ," << n_err << " had errors." << "\n";
		else KALDI_LOG << "Done " << n_done << " n-best entries ," << n_err << " had errors.";
		return (n_done != 0 ? 0 : 1);
	}
