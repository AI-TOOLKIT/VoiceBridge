...
/*
	LatticeAlignWordsLexicon : Convert lattices so that the arcs in the CompactLattice format correspond with
	words (i.e. aligned with word boundaries). This is the newest form, that reads in a lexicon in integer format,
	where each line is (integer id of) word-in word-out phone1 phone2 ... phoneN.
*/
int LatticeAlignWordsLexicon(int argc, char *argv[], fs::ofstream & file_log) {
	try {
		...
		if(file_log)
			file_log << "Successfully aligned " << num_done << " lattices; " << num_err << " had errors." << "\n";
		else KALDI_LOG << "Successfully aligned " << num_done << " lattices; " << num_err << " had errors.";
