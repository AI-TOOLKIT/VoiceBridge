...
/*
	Lattice1best : Compute best path through lattices and write out as FSTs.
*/
int Lattice1best(int argc, char *argv[], fs::ofstream & file_log) {
	try {
		...
		if(file_log)
			file_log << "Done converting " << n_done << " to best path, " << n_err << " had errors." << "\n";
		else KALDI_LOG << "Done converting " << n_done << " to best path, " << n_err << " had errors.";
		return (n_done != 0 ? 0 : 1);
	}
