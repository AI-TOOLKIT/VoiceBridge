...
namespace kaldi {
	...
	bool AppendFeats(const std::vector<Matrix<BaseFloat> > &in,
		std::string utt,
		int32 tolerance,
		Matrix<BaseFloat> *out, fs::ofstream & file_log) {
		// Check the lengths
		...
		if (max_len - min_len > 0) {
			if(file_log)
				file_log << "Length mismatch " << max_len << " vs. " << min_len
				<< (utt.empty() ? "" : " for utt ") << utt
...
/*
	Paste feature files (assuming they have about the same durations, see --length-tolerance), appending the features on each frame.
*/
	...
	try {
		...
		if (ClassifyRspecifier(po.GetArg(1), NULL, NULL)
			...
			for (; !input1.Done(); input1.Next()) {
				...
				if (file_log)
					file_log << "Merging features for utterance " << utt << "\n";
				else
				...
				for (i = 0; i < static_cast<int32>(input.size()); i++) {
					if (input[i]->HasKey(utt)) {
						...
						KALDI_WARN << "Missing utt " << utt << " from input " << po.GetArg(i + 2);
						num_err++;
						break;
					}
				}
				...
				if (!AppendFeats(feats, utt, length_tolerance, &output, file_log)) {
					num_err++;
			...
			if (file_log)
				file_log << "Done " << num_done << " utts, errors on " << num_err << "\n";
			else
			...
			Matrix<BaseFloat> output;
			if (!AppendFeats(feats, "", length_tolerance, &output, file_log))
			...
			if (file_log)
				file_log << "Wrote appended features to " << output_wxfilename << "\n";
			else
