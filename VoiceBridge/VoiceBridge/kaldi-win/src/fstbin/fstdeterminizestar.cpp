...
int fstdeterminizestar(std::string fst_in_str, std::string fst_out_str,
						bool use_log,		//Determinize in log semiring. (Default = false)
						float delta,		//Delta value used to determine equivalence of weights. (Default = kDelta)
						int max_states)		//Maximum number of states in determinized FST before it will abort. (Default = -1)
{
	try {
		using kaldi::int32;
		...
		if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
			...
			for (; !fst_reader.Done(); fst_reader.Next()) {
				...
				try {
					...
					LOGTW_WARNING << " Error during determinization for key " << key;
				}
			}
		}
		return 0;
