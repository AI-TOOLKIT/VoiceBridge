...
static int LoadClusters (const SymbolTable* syms, SymbolMap12M* clusters,
...
void Entry2FSA (const vector<int>& word, VectorFst<Arc>* fsa, size_t maxlen,
  ...
  for (size_t i = 0; i < word.size(); i++) {
    ...
    while (j <= maxlen && i + j < (size_t)word.size()) { //@+zso @BUG corrected: i + j <= (size_t)word.size() changed to i + j < (size_t)word.size()
