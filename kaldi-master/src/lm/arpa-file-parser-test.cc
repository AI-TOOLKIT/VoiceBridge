...
int main(int argc, char *argv[]) {
	...
	//--test-prop1=x.txt
	/*
	std::string prog_name("C://prog.exe");
	std::string test_prop1("--test-prop1=x.txt");

	//--------------------------------------------------------------------------------------
	//NOTE: 1. properties must be passed as "--property-name=property-value" in one 
	//		   string and without space between them!
	...
	arguments.push_back(prog_name);
	arguments.push_back(test_prop1);
	std::vector<char*> _argvP;
	for (const auto& arg : arguments)
		_argvP.push_back((char*)arg.data());
	_argvP.push_back(nullptr);

	size_t argcP = _argvP.size() - 1;
	char ** argvP = _argvP.data();
	//---------------------------------------------------------------------------------------

	//test
	std::string xtest;
	ParseOptions po("");
	po.Register("test-prop1", &xtest, "test");	
	po.Read(argcP, argvP);
