...
struct WeightConvert<Log64Weight, TropicalWeight> {
  TropicalWeight operator()(const Log64Weight &w) const { return (float)w.Value(); }//@+zso (float) 
};
...
struct WeightConvert<Log64Weight, LogWeight> {
  LogWeight operator()(const Log64Weight &w) const { return (float)w.Value(); }//@+zso (float) 
};
