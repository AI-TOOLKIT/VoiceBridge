...
class SymbolTableImpl {
 public:
  ...
  string Find(int64 key) const {
    ...
    if (idx < 0 || idx >= (ssize_t)symbols_.size()) return ""; //@+zso (ssize_t)
  ...
  int64 GetNthKey(ssize_t pos) const {
    if (pos < 0 || pos >= (ssize_t)symbols_.size()) return -1; //@+zso (ssize_t)
...
class SymbolTableIterator {
 public:
  ...
  void Next() {
    ++pos_;
    if (pos_ < (ssize_t)nsymbols_) key_ = table_.GetNthKey(pos_); //@+zso (ssize_t)
  }
